...
```{r}
#| label: tbl-sampling
#| tbl-cap: "Table showing which individuals were sampled in this study (element) and which individuals were sampled in the previous study (KZ_element)."
metadata %>% 
...
Caries were scored as the location on each individual tooth. Multiple locations on
a single tooth were separated with `;`. The size of caries was also
recorded, but not used in further analysis. Large caries that cover multiple
surfaces with an unknown origin were recorded as 'crown'. Caries counts were
extracted based on the number of locations recorded.
